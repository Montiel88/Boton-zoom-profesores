...
function getZoomRecordings($userId, $from = null, $to = null) {
    $cache = new CacheManager();
    // Incluimos el rango de fechas en la llave para evitar servir datos antiguos
    ...
    // Zoom API limita el rango de búsqueda a 1 mes. Si el rango es mayor, debemos iterar.
    ...
    while ($startDate < $endDate) {
        ...
        $nextPageToken = '';
        do {
            $url = "https://api.zoom.us/v2/users/$userId/recordings?page_size=100&from=$chunkStart&to=$chunkEnd";
            if ($nextPageToken) {
                $url .= "&next_page_token={$nextPageToken}";
            }
            ...
            $result = zoomGet($url, $token);
            if ($result['http_code'] !== 200) {
                // Si falla un chunk, seguimos con lo que tenemos
                break;
            }
...
function getPastMeetings($userId, $from = null, $to = null) {
    ...
    // Ampliado a 90 días por defecto, pero respetando los parámetros
    $from = $from ?: date('Y-m-d', strtotime('-90 days'));
    $to = $to ?: date('Y-m-d');

    // Zoom API limita el reporte de reuniones por usuario a un rango de 1 mes.
    ...
    while ($startDate < $endDate) {
        ...
        $nextPageToken = '';
        do {
            $url = "https://api.zoom.us/v2/report/users/$userId/meetings?from=$chunkStart&to=$chunkEnd&page_size=100";
            if ($nextPageToken) {
                $url .= "&next_page_token={$nextPageToken}";
            }
